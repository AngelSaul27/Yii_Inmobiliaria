<?php
    use app\widgets\InmueblesWidget;
    $this->title = 'Inmobiliaria | Encuentra tu hogar';
?>
<? /*Carousel*/ ?>
...
<div class="px-10 py-5">
    <h1 class="font-bold text-2xl">Ultimos Inmuebles</h1>
    <? /* IMPRESIÓN DE INMUEBLES */ ?>
    <div class="grid grid-cols-4 gap-4 py-3 mb-2">
        <?php
            if(isset($lasts) && count($lasts) > 0){
    ...
    <span class="block text-center text-lg"><a href="<?= Yii::getAlias('@web/inmueble')?>">Ver más</a></span>

    <h1 class="font-bold text-2xl">Casas</h1>
    <? /* IMPRESIÓN DE INMUEBLES */ ?>
    <div class="grid grid-cols-4 gap-4 py-3 mb-2">
        <?php
            if(isset($houses) && count($houses) > 0){
                foreach($houses as $house){
                    echo InmueblesWidget::widget(['model' => $house]);
                }
            }else{
                echo '<span class="block w-full font-ligh text-xl">Sin información</span>';
            }
    ...
    <span class="block text-center text-lg"><a href="<?= Yii::getAlias('@web/inmueble/ctg/1')?>">Ver más</a></span>

    <h1 class="font-bold text-2xl">Departamentos</h1>
    <? /* IMPRESIÓN DE INMUEBLES */ ?>
    <div class="grid grid-cols-4 gap-4 py-3 mb-2">
        <?php
            if(isset($deparments) && count($deparments) > 0){
                foreach($deparments as $deparment){
                    echo InmueblesWidget::widget(['model' => $deparment]);
                }
            }else{
                echo '<span class="block w-full font-ligh text-xl">Sin información</span>';
            }
    ...
    <span class="block text-center text-lg"><a href="<?= Yii::getAlias('@web/inmueble/ctg/2')?>">Ver más</a></span>
</div>